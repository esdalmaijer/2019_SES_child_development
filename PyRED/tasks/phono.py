...
class PhonoLoader(TaskLoader):
    
    """Class to process files from the RED phonological awareness (Davis) task.
    """
    
    def __init__(self, data_dir, output_path=None, task_name="DavisTest"):
        
        """Initialises a new PhonoLoader instance to read and process data
        from files generated by the RED phonologial awareness (Davis) task.
    ...
    def load_from_file(self, file_path, delimiter=",", missing=None, \
        ...
        raw = read_behaviour(file_path, delimiter=",", missing=None, \
            auto_typing=True, string_vars=["Option1", "Option2", \
            "ResponseSide", "ResponseCorrect"])
        ...
        raw["ResponseCorrect"] = raw["ResponseCorrect"] == "True"
    ...
    def process_raw_data(self):
        ...
        for i, ppname in enumerate(participants):
            ...
            self.data["n_items"][i]     = len(self.raw[ppname]["QuestionNumber"])
            self.data["n_correct"][i]   = numpy.sum(self.raw[ppname]["ResponseCorrect"].astype(int))
