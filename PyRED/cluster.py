...
def plot_clusters(X, y, xi, yi, zi, xvar, yvar, zvar, ax=None, savepath=None, 
    ...
    if zi is not None:
        ax.set_zlabel(zvar, fontsize=18)
    if closefig:
        if savepath is not None:
            fig.savefig(savepath)
        pyplot.close(fig)
...
def preprocess(X, mode='standardise', impute=None, max_miss_prop=0.3):
    ...
    # Prevent over-writing of X.
    X_ = numpy.copy(X)
    
    # Scale the data.
    for i in range(n_features):
        if mode == "min-max":
            X_[:,i] = preprocessing.minmax_scale(X[:,i])
        elif mode == "standardise":
            X_[:,i] = preprocessing.scale(X[:,i])
    ...
    if impute is None or impute in ["none", "None"]:
        ...
        for i in range(n_subjects):
            ...
            if numpy.sum(missing.astype(int)) > numpy.floor(n_features*max_miss_prop):
                continue
...
def clustering(X, mode="KMEANS", n_clusters=None):
    ...
    # HDBSCAN
        ...
        algorithm = hdbscan.HDBSCAN(min_cluster_size=20)
...
def convenience_clustering(X, mode, max_clusters, outpath, varnames=None, \
    ...
    if len(runs) > 1:
        ...
        fig, ax = pyplot.subplots(nrows=1, ncols=1)
        ax.plot(runs, silhouette_coefficient, 'o-', lw=3, color='#ff69b4')
        ax.set_xlim([0, max(runs)+1])
        ax.set_xticks(range(1, max(runs)+1))
        ax.set_xlabel("Number of clusters", fontsize=18)
        ax.set_ylim([0, 1])
        ax.set_ylabel("Average silhouette coefficient", fontsize=18)
        ...
        if X_original is not None:
            fig, ax = pyplot.subplots(nrows=1, ncols=1)
            ax.plot(runs, silhouette_coefficient_original, 'o-', lw=3, \
                color='#ff69b4')
            ax.set_xlim([0, max(runs)+1])
            ax.set_xticks(range(1, max(runs)+1))
            ax.set_xlabel("Number of clusters", fontsize=18)
        ...
        fig, ax = pyplot.subplots(nrows=1, ncols=1)
        ax.plot(runs, calinski_harabaz, 'o-', lw=3, color='#ff69b4')
        ax.set_xlim([0, max(runs)+1])
        ax.set_xticks(range(1, max(runs)+1))
        ax.set_xlabel("Number of clusters", fontsize=18)
        ax.set_ylabel("Calinski-Harabaz index", fontsize=18)
